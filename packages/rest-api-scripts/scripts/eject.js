...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put rest-api-scripts in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove rest-api-scripts reference since they're getting a copy of the types in their project
        content =
          content
            // Remove rest-api-scripts types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"rest-api-scripts".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove rest-api-scripts and rest-api-scripts binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      const windowsCmdFilePath = path.join(
        ...
        'node_modules',
        '.bin',
        'rest-api-scripts.cmd'
      );
      let windowsCmdFileContent;
      if (process.platform === 'win32') {
        // https://github.com/facebook/create-react-app/pull/3806#issuecomment-357781035
        // Yarn is diligent about cleaning up after itself, but this causes the rest-api-scripts.cmd file
